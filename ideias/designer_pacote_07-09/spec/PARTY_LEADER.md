...
> Geometria, hierarquia de nós e ancoragem: **`POSICIONAMENTO.md`**. Este arquivo
> cobre o modelo do líder; aquele cobre o encaixe.
...
| anel (border interna) | **sempre a cor do elemento**, líder incluído |
...
Distinção do líder **sem mudar tamanho nem cor**: placa `LEADER` + halo na cor do
elemento (`filter: drop-shadow(0 0 8px <elem-light> 45%)` no HTML → em Godot, um
`CanvasItem` com material de glow ou um `NinePatch` de halo atrás do card).
O anel dourado saiu: o card do líder é nature e o dourado o fazia ler como light.
Único elemento dourado que resta é a borda da placa LEADER, que é chrome de UI,
não cor de elemento.
...
fundo = cor do elemento (líder incluído)

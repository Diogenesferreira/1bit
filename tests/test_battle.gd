...
func run() -> void:
	...
	c.select_card(0)
	c.select_card(3)
	expect(c.state.party_hp == hp - 195 and c.state.enemy_countdown == 3, "Enemy attacks only every third spent turn")
	c.load_test_hand()
	c.select_card(0)
	c.select_card(1)
	c.select_card(2)
	...
	expect(c.state.last_chain[0].critical and c.state.last_chain[0].damage == 360, "444 critical contribution")
	expect(c.state.last_chain[1].critical and c.state.last_chain[1].damage == 351, "234 critical with second-chain bonus")
	...
	c.select_card(2)
	await wait_chain(c)
	expect(c.state.last_chain[0].damage == 1600, "All wild contributes fully to five allies")
	c.queue_free()
	await process_frame
